...
<body>
							...
							<div class="modal-content">
								<div class="modal-header">
									...
									<div class="modal-body col-md-12">						
                                        <form id="formcmdcompany" method="POST" Action="savecmdclient.php" class="form-horizontal col-md-12" autocomplete="off">
										@csrf 
											<div class="row">
				...
				<div class="card-box mb-30">
					...
					<div class="pb-20">
						<table class="data-table table stripe hover nowrap">
							...
							<tbody>
							@foreach ($commandecompany as $item)
										...
										@csrf 
											<div class="row">
                                                <div class="col-md-6  mt-3 text-left">
                                                    ...
                                                    <div class="form-group">
                                                        <label for="fournisseur">Fournisseur</label>
                                                        <select class="form-control" name="fournisseur_id" id="fournisseur_id" value="{{$item->fournisseur_id}}">
                                                            <optgroup >																				
                                                                <option value="{{$item->fournisseur_id}}">{{$item->fournisseur_id}}</option>													
                                                            </optgroup>	
                                                        </select>
                                                    </div>
                                                    <div class="form-group">
                                                    ...
                                                    <div class="form-group">
                                                        <label for="quantite">Quantité</label>
                                                        <input type="number" class="form-control" min="0" placeholder="quantité" value="{{$item->quantite}}" name="quantite" id="quantite" oninput="this.value = Math.abs(this.value)" required/>
                                                    </div>                                                    
                                                </div>
                                                <div class="col-md-6  mt-3 text-left">  
                                                    <div class="form-group">
                                                        <label for="prix">Prix unitaire</label>
                                                        <input type="number" class="form-control" placeholder="Prix unitaire" value="{{$item->prix}}" name="prix" id="prix" min="0" oninput="this.value = Math.abs(this.value)" required/>
                                                    </div>                       
                                                    <div class="form-group">
                                                        <label for="devise">Devise</label>
                                                    ...
                                                    <div class="form-group">
                                                        <label for="date">Date</label>
                                                        <input type="date" class="form-control" value="{{$item->datecmd}}"  name="datecmd" id="datecmd" required/>
                                                    </div>                        
                                                </div>
                                            </div>
                                            <div class="form-group">                               
									...
									<td>
										<div class="dropdown">
											...
											<div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">												
												<a class="dropdown-item" data-toggle="modal" data-target="#edit{{$item->id}}" href="{{'/commandecompany/edit/'.$item->id}}".$id><i class="dw dw-edit2"></i> Edit</a>
												<a class="dropdown-item" onclick= "return (confirm(' Voulez-vous supprimer vraiment cette information ?'));"  href="{{'/commandecompany/delete/'.$item->id}}"><i class="dw dw-delete-3"></i> Delete</a>											
											</div>
										</div>
									</td>
								</tr>
	...
	<script src="{{url('assets\vendors\scripts\datatable-setting.js')}}"></script>
		<script type="text/javascript">
			$('#formcmdcompany').submit(function(e){

				e.preventDefault();

				$.ajax({
					url:'{{ route("commandecompany.store")}}',
					method: 'POST',
